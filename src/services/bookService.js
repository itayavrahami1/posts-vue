...
const API_KEY_STR = '&key=AIzaSyCnrcFHEWtsdej9RrPgjrCeYLtfP6CJhgA'; //flowers+inauthor:keyes&key=yourAPIKey
...
function query() {
  ...
  return Promise.resolve(JSON.parse(JSON.stringify(booksToReturn)));
}
...
function save(bookToSave) {
  ...
  } else if (bookToSave.volumeInfo) { // in case off adding book from google books
    bookToSave = _setGoogleBookToSave(bookToSave)
...
async function getGooglsBookByTitle(title) {
  const searchURL = `${GOOGLE_BOOKS_URL}${title}+intitle:${title}${API_KEY_STR}`
  const googleBooksReq = await axios.get(searchURL);
  return Promise.resolve(googleBooksReq.data.items);
}
...
function _setGoogleBookToSave(book) {
  const bookToAdd = {
    "_id": book.id,
    ...
    "thumbnail": book.volumeInfo.imageLinks.thumbnail || `https://api.adorable.io/avatars/120/${bookToSave.name}.png`,
  ...
  if (book.saleInfo.saleability !== 'NOT_FOR_SALE') {
    ...
    let isOnSale = (book.saleInfo.saleability === 'FOR_SALE') ? true : false;
    bookToAdd["listPrice"] = {
      "amount": 109,
      "currencyCode": "EUR",
      "isOnSale": isOnSale
...
function getNextPrevIds(id) {
  const currIdx = gBooks.findIndex(book => { return book._id === id })
  const nextBookId = gBooks[currIdx + 1]?._id || gBooks[0]._id
  const prevBookId = gBooks[currIdx - 1]?._id || gBooks[gBooks.length - 1]._id
  return [prevBookId,nextBookId]
}
